...
export class Smoke{
  ...
  init() {
    ...
    // console.log('geometry 1',this.geometry)
    ...
    shader.fragmentShader = shader.fragmentShader.replace('void main() {', fragment1)
    shader.fragmentShader = shader.fragmentShader.replace('}', fragment2)
  ...
  setAttribute(
    ...
    // console.log('geometry 2',this.geometry)
 }
  ...
  update() {
    ...
    this.smokes = this.smokes.filter(item => {
      ...
      // console.log('size ',item.size,item.scale,item.opacity)
